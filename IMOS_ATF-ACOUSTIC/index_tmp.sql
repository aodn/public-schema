...
-- read geom from dataframe - trajectory
SELECT AddGeometryColumn('animal_tracking_acoustic_qc','index_tmp','geom',4326,'LINESTRING',2);
UPDATE index_tmp
...
insert into detection_file_index (tag_deployment_id, transmitter_id, species, tag_project,
                                  time_coverage_start, time_coverage_end, url, size, geom)
SELECT tag_deployment_id, transmitter_id, species, tag_project, time_coverage_start, time_coverage_end,
       url, size, geom
FROM index_tmp

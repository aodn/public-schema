...
WITH grouped AS (
    ...
    SELECT trip_code, genus, biovolume_um3l  FROM default_biovolumes
    UNION ALL
    SELECT trip_code, genus, biovolume_um3l  FROM grouped
), regrouped AS (
...
SELECT m.*,
       coalesce(p.biovolumes, '{"Acanthoica": 0}'::jsonb) AS biovolumes

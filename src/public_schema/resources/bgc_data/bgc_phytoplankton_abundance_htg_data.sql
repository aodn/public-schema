-- Phytoplankton Higher Taxonomic Groups (HTG) abundance product
CREATE OR REPLACE TABLE bgc_phytoplankton_abundance_htg_data AS
WITH grouped AS (
    ...
    SELECT trip_code, methods, taxon_group, cell_l  FROM default_abundances
    UNION ALL
    SELECT trip_code, methods, taxon_group, cell_l  FROM grouped
), regrouped AS (
    -- create a single row per trip/group, making sure observed abundances override default values and filter out extra methods rows
    SELECT dg.trip_code,
           dg.methods,
           dg.taxon_group,
           max(dg.cell_l) AS cell_l

-- Materialized view for CPR Phytoplankton Genus abundance product
...
CREATE MATERIALIZED VIEW cpr_phytoplankton_abundance_genus_data AS
WITH grouped AS (
    ...
    SELECT sample, genus, phyto_abundance_m3  FROM default_abundances
    UNION ALL
    SELECT sample, genus, phyto_abundance_m3  FROM grouped
), regrouped AS (
    ...
    SELECT sample,
           jsonb_object_agg(genus, phyto_abundance_m3) AS abundances
    FROM regrouped
    GROUP BY sample
)

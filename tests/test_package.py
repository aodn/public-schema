...
def test_bgc_data_resource_accessible():
    resource = files("public_schema.resources.bgc_data") / "bgc_chemistry.dataresource.yaml"
    assert resource.is_file(), "bgc_chemistry.dataresource.yaml not found in package"


def test_cpr_data_resource_accessible():
    resource = files("public_schema.resources.cpr_data") / "cpr_phyto_raw.dataresource.yaml"
